...
class BoxService{
  static Future<Map<String, dynamic>> index({BuildContext context, scaffoldKey, AppDatabase db}) async {
    ...
    var response = await http.post(Utils.URL + "/api/boxes", body: json.encode(map), headers: Utils.header);
  ...
  static Future<Map<String, dynamic>> store({BuildContext context, GlobalKey<ScaffoldState> scaffoldKey, Caja caja, AppDatabase db}) async {
    var map = Map<String, dynamic>();
    // cliente.toJson();
    // map["servidor"] = await Db.servidor();
  ...
  static Future<Map<String, dynamic>> open({BuildContext context, GlobalKey<ScaffoldState> scaffoldKey, Caja caja, AppDatabase db}) async {
    var map = Map<String, dynamic>();
    // cliente.toJson();
    // map["servidor"] = await Db.servidor();
    map["data"] = caja.toJson();
    ...
    // print("map: ${map}");
    // var jwt = await Utils.createJwt(map);
    var response = await http.post(Utils.URL + "/api/boxes/open", body: json.encode(map), headers: Utils.header);
    int statusCode =response.statusCode;
    ...
    if(statusCode < 200 || statusCode > 400){
      ...
      print("Error: ${parsed}");
      if(context != null)
        Utils.showAlertDialog(content: "Error BoxService open: ${parsed["message"]}", title: "Error", context: context);
      else
        Utils.showSnackBar(content: "Error BoxService open: ${parsed["message"]}", scaffoldKey: scaffoldKey);
      throw Exception("Error Servidor BoxService open: ");
    }
  ...
  static Future<Map<String, dynamic>> delete({BuildContext context, GlobalKey<ScaffoldState> scaffoldKey, Caja caja, AppDatabase db}) async {
    var map = Map<String, dynamic>();
    // cliente.toJson();
    // map["servidor"] = await Db.servidor();
    map["data"] = caja.toJson();
